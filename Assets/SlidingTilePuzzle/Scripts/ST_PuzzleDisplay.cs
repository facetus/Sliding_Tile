...
public class ST_PuzzleDisplay : MonoBehaviour
{
    ...
    public static event Action OnAutoSolve;
	// this puzzle texture.
	...
	public bool Complete = false; // has the puzzle been completed?
	// ADJUSTMENTS FOR AR APP
	//public Transform arCameraTransform;
	//public float xOffset = 2;
	//public float yOffset = -2.0f;
	//public float xRotationOffset = 90.0f;


	private bool finishedJizzle;
	...
	void Update()
	{
		...
		// V.01 WORKING ::
		// SETS THE PUZZLE FIXED IN THE AR CAMERA VIEW OF THE PLAYER, HAS A BIT OF A DELAY
		//if (arCameraTransform != null)
		//{
		//	Vector3 xOffsetVector = arCameraTransform.right * xOffset;
		//	Vector3 yOffsetVector = arCameraTransform.up * yOffset;
		//	Vector3 targetPosition = arCameraTransform.position + arCameraTransform.forward * 19 + xOffsetVector + yOffsetVector;
		//	this.transform.position = targetPosition;
		//
		//	Quaternion offsetRotation = Quaternion.Euler(xRotationOffset, 0, 0);
		//	this.transform.rotation = arCameraTransform.rotation * offsetRotation;
		//}
	}
	...
	public void JinglePuzzleMethod()
	{
		StartCoroutine(JugglePuzzle());
	}
	...
	public void CreatePuzzleTiles()
	{
		...
		// Spawn the tiles into an array in the correct order.
		for (int j = Height - 1; j >= 0; j--)
		{
			for (int i = 0; i < Width; i++)
			{
				// Calculate the position of this tile, centered around Vector3(0.0f, 0.0f, 0.0f).
				Position = new Vector3(
					((Scale.x * i) - (Scale.x * (Width / 2.0f))) * (10.0f + SeperationBetweenTiles),
					0.0f,
					((Scale.z * j) - (Scale.z * (Height / 2.0f))) * (10.0f + SeperationBetweenTiles)
				);
				...
				//TileDisplayArray[i, j] = Instantiate(Tile, Position, Quaternion.Euler(180.0f, 180.0f, 0.0f)) as GameObject;
				TileDisplayArray[i, j] = Instantiate(Tile, Position, Quaternion.Euler(180.0f, 180.0f, 0.0f)) as GameObject;
				TileDisplayArray[i, j].transform.SetParent(this.transform, false);

				// Set the array and grid locations for each tile.
				...
				// Set the target position for each tile.
				thisTile.LaunchPositionCoroutine(Position);

				// Calculate the correct texture coordinates for this tile.
				float tileWidth = 1.0f / Width;
				float tileHeight = 1.0f / Height;
				Vector2 offset = new Vector2(tileWidth * i, 1.0f - tileHeight * (j + 1));

				// Create a new material using the defined shader.
				Material thisTileMaterial = new Material(PuzzleShader);

				// Apply the puzzle image to it and set the correct texture coordinates.
				thisTileMaterial.mainTexture = PuzzleImage;
				thisTileMaterial.mainTextureOffset = offset;
				thisTileMaterial.mainTextureScale = new Vector2(tileWidth, tileHeight);
